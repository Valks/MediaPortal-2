<<<<<<< HEAD:MediaPortal/Source/Core/MediaPortal.Common/Services/ServerCommunication/MPnP/MPnPClientControlPoint.cs
...
namespace MediaPortal.Common.Services.ServerCommunication.MPnP
{
  ...
  /// </summary>
  public class MPnPClientControlPoint
  {
    ...
    protected MPnPContentDirectoryServiceProxy _contentDirectoryService = null;
    protected MPnPServerControllerServiceProxy _serverControllerService = null;
    protected MPnPResourceInformationServiceProxy _resourceInformationService = null;
    protected MPnPUserProfileDataManagementServiceProxy _userProfileDataManagementService = null;
    ...
    public MPnPClientControlPoint(string homeServerSystemId)
    {
      ...
      _networkTracker.RootDeviceAdded += OnMPnPRootDeviceAdded;
    }
    ...
    /// <summary>
    /// Gets or sets the system ID of the MediaPortal 2 server, which is this MPnP client's homeserver.
    ...
    public MPnPContentDirectoryServiceProxy ContentDirectoryService
    {
    ...
    public MPnPServerControllerServiceProxy ServerControllerService
    {
      get { return _serverControllerService; }
    }

    public MPnPUserProfileDataManagementServiceProxy UserProfileDataManagementService
    {
    ...
    /// <summary>
    /// Registers a delegate for creating an additional MPnP service proxy object upon device connection. This control point keeps track of the
    ...
    /// </summary>
    /// <remarks>
    /// This method will typically be used by plugins which want to register custom MPnP service proxy objects. The service proxy object, which is created
    ...
    void OnMPnPRootDeviceAdded(RootDescriptor rootDescriptor)
    {
    ...
    protected void TryConnect(RootDescriptor rootDescriptor)
    {
      ...
      lock (_networkTracker.SharedControlPointData.SyncObj)
      {
        ...
        if (deviceUuid == _homeServerSystemId)
          ServiceRegistration.Get<ILogger>().Debug("MPnPClientControlPoint: Found MP2 home server '{0}' (system ID '{1}') at host '{2}' (IP address: '{3}')",
              friendlyName, deviceUuid, system.HostName, system.Address);
        else
        {
          ServiceRegistration.Get<ILogger>().Debug("MPnPClientControlPoint: Found foreign MP2 server '{0}' (system ID '{1}') at host '{2}' (IP address: '{3}')",
              friendlyName, deviceUuid, system.HostName, system.Address);
          return;
        }
        try
        {
          connection = _connection = _controlPoint.Connect(rootDescriptor, deviceUuid, MPnPExtendedDataTypes.ResolveDataType);
        }
        catch (Exception e)
        {
          ServiceRegistration.Get<ILogger>().Warn("MPnPClientControlPoint: Error connecting to MPnP MP2 backend server '{0}'", e, deviceUuid);
          return;
        }
      }
      connection.DeviceDisconnected += OnMPnPDeviceDisconnected;
      try
      {
        CpService cdsStub = connection.Device.FindServiceByServiceId(MPnPTypesAndIds.CONTENT_DIRECTORY_SERVICE_ID);
        if (cdsStub == null)
          throw new InvalidDataException("ContentDirectory service not found in device '{0}' of type '{1}:{2}'",
              deviceUuid, MPnPTypesAndIds.BACKEND_SERVER_DEVICE_TYPE, MPnPTypesAndIds.BACKEND_SERVER_DEVICE_TYPE_VERSION);
        CpService risStub = connection.Device.FindServiceByServiceId(MPnPTypesAndIds.RESOURCE_INFORMATION_SERVICE_ID);
        if (risStub == null)
          throw new InvalidDataException("ResourceAccess service not found in device '{0}' of type '{1}:{2}'",
              deviceUuid, MPnPTypesAndIds.BACKEND_SERVER_DEVICE_TYPE, MPnPTypesAndIds.BACKEND_SERVER_DEVICE_TYPE_VERSION);
        CpService scsStub = connection.Device.FindServiceByServiceId(MPnPTypesAndIds.SERVER_CONTROLLER_SERVICE_ID);
        if (scsStub == null)
          throw new InvalidDataException("ServerController service not found in device '{0}' of type '{1}:{2}'",
              deviceUuid, MPnPTypesAndIds.BACKEND_SERVER_DEVICE_TYPE, MPnPTypesAndIds.BACKEND_SERVER_DEVICE_TYPE_VERSION);
        CpService updmStub = connection.Device.FindServiceByServiceId(MPnPTypesAndIds.USER_PROFILE_DATA_MANAGEMENT_SERVICE_ID);
        if (updmStub == null)
          throw new InvalidDataException("UserProfileDataManagement service not found in device '{0}' of type '{1}:{2}'",
              deviceUuid, MPnPTypesAndIds.BACKEND_SERVER_DEVICE_TYPE, MPnPTypesAndIds.BACKEND_SERVER_DEVICE_TYPE_VERSION);
        lock (_networkTracker.SharedControlPointData.SyncObj)
        {
          _contentDirectoryService = new MPnPContentDirectoryServiceProxy(cdsStub);
          _resourceInformationService = new MPnPResourceInformationServiceProxy(risStub);
          _serverControllerService = new MPnPServerControllerServiceProxy(scsStub);
          _userProfileDataManagementService = new MPnPUserProfileDataManagementServiceProxy(updmStub);
        }
        ...
        foreach (AdditionalServiceRegisterDlgt additionalServiceRegistration in _additionalServiceRegistrations)
        {
          ...
          {
            ServiceRegistration.Get<ILogger>().Warn("MPnPClientControlPoint: Error registering user service for MPnP MP2 backend server '{0}'", e, deviceUuid);
          }
        }
        lock (_networkTracker.SharedControlPointData.SyncObj)
      ...
      {
        ServiceRegistration.Get<ILogger>().Warn("MPnPClientControlPoint: Error connecting to services of MPnP MP2 backend server '{0}'", e, deviceUuid);
        connection.DeviceDisconnected -= OnMPnPDeviceDisconnected;
    ...
    void OnMPnPDeviceDisconnected(DeviceConnection connection)
    {
      ...
      foreach (UPnPServiceProxyBase service in servicesToDispose)
      {
        ...
        {
          ServiceRegistration.Get<ILogger>().Warn("MPnPClientControlPoint: Error disposing additional service '{0}'", service, e);
        }
      }
...
>>>>>>> c59b54e842c7f67c2604e643a3a7160bb2dd8506:MediaPortal/Source/Core/MediaPortal.Common/Services/ServerCommunication/UPnPClientControlPoint.cs
